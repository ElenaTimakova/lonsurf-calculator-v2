...
export const LEGAL_DISCLAIMER_BEFORE =
  'Важно: Данный интерактивный инструмент (калькулятор подбора доз препаратов) является вспомогательным и служит исключительно в целях ознакомления медицинского работника с ориентировочными показателями. Для медицинского применения ЛП Лонсурф® трифлуридин + [типирацил] сотруднику здравоохранения необходимо ознакомиться с ';

export const LEGAL_DISCLAIMER_LINK_TEXT = 'ОХЛП-№(012426)-(РГ-RU)';

export const LEGAL_DISCLAIMER_AFTER =
  '. Указанную в действующей ОХЛП информацию следует всегда считать приоритетной.';

export const SMPC_PDF_FILE = '20251113_lonsurf_smpc_rus.pdf';
